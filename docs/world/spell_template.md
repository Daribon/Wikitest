...
- <a id="f-auraDescription"></a><a id="f-description"></a><a id="f-name"></a><a id="f-nameSubtext"></a>**`name`**, **`nameSubtext`**, **`description`**, **`auraDescription`** - Display text
  (name, subtext/rank line, tooltip body, aura tooltip). Only `name` and `nameSubtext` are read by the server; `description` / `auraDescription` are client-DBC/informational columns.
- <a id="f-auraDescriptionFlags"></a><a id="f-descriptionFlags"></a><a id="f-nameFlags"></a><a id="f-nameSubtextFlags"></a>**`nameFlags`**, **`nameSubtextFlags`**, **`descriptionFlags`**, **`auraDescriptionFlags`** -
  Locale mask for the Aura Description (tooltip) text; not read by server.

### Identity Extras

- <a id="f-castUI"></a>**`castUI`** - DBC passthrough column; not read by server.
...
- <a id="f-modelNextSpell"></a>**`modelNextSpell`** - DBC passthrough field; not used by the server.
...
- <a id="f-spellVisual1"></a><a id="f-spellVisual2"></a>**`spellVisual1` / `spellVisual2`** - Cast visual ids (`SpellVisual.dbc`);
  `spellVisual2` is skipped by the server (not used).
...
- <a id="f-stanceBarOrder"></a>**`stanceBarOrder`** - Position in the shapeshift bar (DBC passthrough; not read by the server).
...
- <a id="f-minFactionId"></a><a id="f-minReputation"></a>**`minFactionId`** / **`minReputation`** - Reputation requirement for the spell to work (DBC passthrough; not read by the server).
- <a id="f-requiredAuraVision"></a>**`requiredAuraVision`** - Caster needs an aura allowing them to see stealth/invisible
  targets of this kind (DBC passthrough; not read by the server).
