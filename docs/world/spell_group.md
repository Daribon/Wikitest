...
- <a id="f-group_spell_id"></a>**`group_spell_id`** - Primary Key. Companion group spell id; ordered by the server but otherwise unused by the current core.
- <a id="f-spell_id"></a>**`spell_id`** - Primary Key. Spell ID. (see [`spell_template`](spell_template.md).entry)
  Note: the server also supports a *negative* value meaning "reference another group" (`SpellMgr.cpp`),
