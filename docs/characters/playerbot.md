...
Bot configuration read by the built-in bot system at startup (read-only - the server never writes this table).
...
- <a id="f-comment"></a>**`comment`** - Descriptive label (not used by server).
